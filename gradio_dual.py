...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Column():
        with gr.Group(elem_id="main-image"):
            ...
            with gr.Column(visible=False) as gen_prompt_vis:
                with gr.Row():
                    img_name_box = gr.Textbox(label="image_name", value="test2")
                    save_example = gr.Button("save current case to examples 😺", elem_id="prompt_button", interactive=True)
                general_prompt = gr.Textbox(value='', label="(3) Textual Description for the entire image", interactive=True)
                general_prompt_dual = gr.Textbox(value='', label="Please change 'game scene' to 'empty terrain'", interactive=True)
                # artifacts, human, waterfall, ugly, disproportionate, distorted, low res, rundown
                ...
                negative_prompt_dual = gr.Textbox(value='distorted, low res', \
                                             label="Negative prompt for the empty terrain: distorted, low res", interactive=True)
                with gr.Accordion("(4) Tune the hyperparameters", open=False):
                    ...
                    with gr.Row():
                        ...
                        bsz_ = gr.Slider(label="Number of Samples to generate", minimum=1, maximum=5, value=2, step=1)
                    with gr.Row():
                        lora_w_ = gr.Slider(label="The scale of LoRA", minimum=0, maximum=5, value=1.0, step=0.1)
                        guidance_ = gr.Slider(label="Inference guidance", minimum=0, maximum=10., value=9., step=0.1)
                    with gr.Row():
                        ip_strength_ = gr.Slider(label="ip adapter strength", minimum=0, maximum=3., value=0., step=0.01)
                        separate_steps = gr.Slider(label="The step to separate the dual input", minimum=0, maximum=100, value=16, step=1)
                    with gr.Row():
                        trick_seed = gr.Slider(label="Whether to use the trick seed", minimum=0, maximum=1, value=0, step=1)
                        debug_attn = gr.Checkbox(label="Debug to save Attn")
                    with gr.Row():
                    ...
                    with gr.Row():
                        beta_value = gr.Slider(label="Strength to control the value", minimum=0, maximum=10, value=2, step=0.1)
                        beta_num_value = gr.Slider(label="Top K to control the value", minimum=0, maximum=20, value=0, step=1)
                    with gr.Row():
                        switch_prompt = gr.Slider(label="switch the replacement of prompt, 0:original, 1:single, 2:group", minimum=0, maximum=3, value=1, step=1)
    ...
    debug_attn.change(process_checkbox, inputs=debug_attn)
    ...
    get_genprompt_run.click(process_prompts, inputs=[binary_matrixes, *prompts], outputs=[gen_prompt_vis, general_prompt, general_prompt_dual], queue=False)
    save_example.click(save_examples_json, inputs=[image_sketch, img_name_box, seed_, general_prompt, *prompts], outputs=img_name_box, queue=False)
    
    final_run_btn.click(process_generation, inputs=[image_style, image_sketch, layout_path, binary_matrixes, seed_, creg_, sreg_, sizereg_, dense_step_, bsz_, \
                                                    general_prompt, general_prompt_dual, negative_prompt, negative_prompt_dual, img_name_box, \
                                                    feat_value, feat_num, beta_value, beta_num_value, s_prompt, switch_prompt, switch_balance, \
        control_scale_, guidance_, infer_steps_, control_start_, control_end_, lora_w_, ip_strength_, separate_steps, trick_seed, *prompts], outputs=[image_sketch_control, out_image])

    gr.Examples(
        examples=generate_examples(val_layout, val_prompt),
        inputs=[layout_path, all_prompts, seed_],
        outputs=[post_sketch, binary_matrixes, *color_row, *colors, *prompts, gen_prompt_vis, general_prompt, general_prompt_dual, seed_, image_sketch],
        fn=process_example,
        run_on_click=True,
        label='😺 Examples 😺',
    )
...
demo.launch(server_name="0.0.0.0", server_port=30009)
