...
nltk.data.path.append('/mnt/afse1/zhenhong/downloads/nltk_data-gh-pages/packages')
# nltk.download('wordnet')

def return_noun_indices(DECODED_PROMPTS, is_bare=False):
    # pdb.set_trace()
    ...
    # 检查 "bare" 是否在文本中
    if is_bare:
        if "bare" in DECODED_PROMPTS:
            bare_index = DECODED_PROMPTS.index("bare")  # 加2调整为DECODED_PROMPTS整体的索引
        if "empty" in DECODED_PROMPTS:
            bare_index = DECODED_PROMPTS.index("empty")  # 加2调整为DECODED_PROMPTS整体的索引
        if bare_index not in noun_indices:
            noun_indices.append(bare_index)
            noun_indices.sort()
...
def process_example(layout_path, all_prompts, seed_, en_gradio=1):
    ...
    if en_gradio==0:  
        return binary_matrixes, all_prompts[0], prompts
    elif en_gradio==1:  
        return [gr.update(visible=True), binary_matrixes, *visibilities, *colors, *prompts,
            ...
            gr.update(value=all_prompts[0].replace("game scene", "empty terrain")), 
            int(seed_), Image.open(layout_path)]
    elif en_gradio==2: 
        return all_prompts[0], visibilities, colors, prompts

def process_example_old(layout_path, all_prompts, seed_):
    ...
    return [gr.update(visible=True), binary_matrixes, *visibilities, *colors, *prompts,
            gr.update(visible=True), gr.update(value=all_prompts[0]), int(seed_), Image.open(layout_path)]